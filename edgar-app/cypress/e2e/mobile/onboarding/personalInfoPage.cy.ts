...
describe('Personal Info Page tests', () => {
	...
	describe('Good number of elements - Mobile', { testIsolation: false }, () => {
		before(() => {
	...
	describe('Good content on elements - Mobile', { testIsolation: false }, () => {
		before(() => {
			cy.window().then((win) => win.localStorage.setItem('token', authToken));
			cy.visit('/onboarding/personal');
			cy.wait(1000);
	...
	describe('Visible elements - Mobile', { testIsolation: false }, () => {
		before(() => {
			cy.window().then((win) => win.localStorage.setItem('token', authToken));
			cy.visit('/onboarding/personal');
			cy.wait(1000);
