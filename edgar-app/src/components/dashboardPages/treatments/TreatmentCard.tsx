...
const TreatmentCard = ({ treatment }: { treatment: PatientMedicineType }) => {
	...
	const getPeriodIconColor = (period: TreatmentPeriodType): string => treatment.periods.includes(period) ? "blue.700" : "grey.300";
	const getPeriodIconWidth = (period: TreatmentPeriodType): string => periodIconWidthMap[period];

	return (
		<Skeleton isLoaded={!isLoading} w="100%">
			<VStack bg="blue.50" borderRadius="8px" p="8px 16px" border="2px solid" borderColor="blue.200" align="start" w="100%">
				<HStack spacing="4px">
					<Text size="boldLg">{medicineInfo?.name}</Text>
					<Text size="md">-</Text>
					<Text size="md">{treatment?.quantity} {medicineInfo?.unit}</Text>
				</HStack>
              ...
              />
					...
					<HStack spacing="6px">
						{Object.keys(periodIconMap).map((period) => (
							<Icon key={period} as={periodIconMap[period as TreatmentPeriodType]} w={getPeriodIconWidth(period as TreatmentPeriodType)} h="21px" color={getPeriodIconColor(period as TreatmentPeriodType)} />
						))}
					</HStack>
				</VStack>
			</VStack>
