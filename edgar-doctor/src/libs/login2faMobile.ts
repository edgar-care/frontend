...
import type { ReceiveMobileConnectionMessage } from 'types/app/login-2fa/Login2faMobileClassType';
...
class Login2faMobile {
	...
	public askMobileConnection = (email: string, password: string) => {
		try {
			...
			this.socket.send(
				JSON.stringify({ action: 'askMobileConnection', uuid: this.uuid, type: 'd', email, password }),
			);
